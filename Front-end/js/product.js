...
function getAll() {
    $.ajax({
        url: "http://192.168.100.4:8086/products/allProducts",
        type: "GET",
...
function getSearch(valor) {
    $.ajax({
        url: "http://192.168.100.4:8086/products/getProduct/" + valor,
        type: "GET",
        contentType: "application/json; charset=utf-8",
        cache: false,
        timeout: 600000,
    ...
    if (data != null && data) {
        for (var i = 0; i < data.length; i++) {
            ...
            if (puntos <= 0) {
              ...
              +    "<div class=\"center\">"
              +      "<h4><a onclick=\"goToDetail(" + data[i].id +")\" href=\"it_shop_detail.html\" >" + data[i].nombre + "</a></h4>"
              +    "</div>"
              +    "<div class=\"starratin\">"
              +      stars
              +    "</div>"
